...
namespace ContentTools
{
    public static class Utils
    {
        ...
        public static bool Vector2NearEqual(Vector2 a, Vector2 b, float eps = Epsilon)
        {
            return
                MathF.Abs(a.X - b.X) <= eps &&
                MathF.Abs(a.Y - b.Y) <= eps;
        }
        public static bool Vector3NearEqual(Vector3 a, Vector3 b, float eps = Epsilon)
        {
            return
                MathF.Abs(a.X - b.X) <= eps &&
                MathF.Abs(a.Y - b.Y) <= eps &&
        ...
        public static bool Vector4NearEqual(Vector4 a, Vector4 b, float eps = Epsilon)
        {
            return
                MathF.Abs(a.X - b.X) <= eps &&
                MathF.Abs(a.Y - b.Y) <= eps &&
